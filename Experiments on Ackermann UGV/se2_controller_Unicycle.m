...
block.NumDialogPrms = 7;  % 由block输入参数数目为4，输入参数为目标位置pd(2*1)，目标朝向theta_d，最小转弯半径，最大线速度，CVF设计参数k1,k2,k3

%% Register number of input and output ports
block.NumInputPorts  = 2;   %控制器输入两路，一路为当前位置p(2*1)，一路为当前姿态角theta
block.NumOutputPorts = 5;   %控制器输出两路，一路为体坐标系线速度v(2*1)，一路为角速度w
...
block.OutputPort(3).Dimensions       = 1;     %角速度增益kw
block.OutputPort(4).Dimensions       = 1;     %参考姿态
...
function Output(block)
    ...
    vx = (1-exp(-0.3*t))*(0 + kv*tanh(norm(p-pd)/c_p+abs(theta_e)/c_theta));
    v = [vx;0];
    dot_p = R*v;
    ...
    ending_bool = vx < v_max/10 & d_e < rho/3;
    if ending_bool 
